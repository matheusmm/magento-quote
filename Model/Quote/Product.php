...
class Intelipost_Quote_Model_Quote_Product
{
	...
	public function fetchProductQuote($items, $dimensions)
	{
		...
		for ($i = 0; $i < $productsCount; $i ++)
		{
		    ...
		    if(!strcmp(Mage::getStoreConfig('carriers/intelipost/price_config'),'product'))
            {
            	...
            	{
            		$productFinalPrice = $this->_simpleProducts[$i][$i];
            	}
            	else
            	{
	...
	private function getSimpleProducts($items)
	{
	    ...
	    foreach ($items as $child)
	    {
	    	...
	    	if ($productP->getTypeId() == 'bundle') 
	    	{
				...
				$subTotalProduct = $this->getSubTotalProduct($targetItem->getProductId());
	    		$bundleItemsQty = $this->getOneBundleItemsQty($productP->getId());
	    		$singleProductQty = $qty;

	    		$sharedPrice = (($subTotalProduct) / ($bundleItemsQty * $singleProductQty));
	    	}

	        $product = Mage::getModel ('catalog/product')->load ($child->getProductId());
	        
	        $this->_simpleProducts [$j] = $product;
	        $this->_simpleProducts[$j][$j] = $sharedPrice;
	        $this->_productsQty [$j] = (int)$qty;	        
	        $j = $j + 1;
	        $this->_bundleOptions = array();
	    }
    ...
    public function getOneBundleItemsQty($bundledId)
    {
    	$id = $bundledId;
 		$productBundle = Mage::getModel('catalog/product')->setStoreId(Mage::app()->getStore()->getId())->load($id);
 		$collection = $productBundle->getTypeInstance(true)
 		->getSelectionsCollection($productBundle->getTypeInstance(true)->getOptionsIds($productBundle), $productBundle);
    ...
    public function getCartQty($id)
    {
		...
		{
		    $_product = $singleItem->getProduct();
		    $cartId = $_product->getId();
		    if ($cartId == $id){
		    	$cartQty = $singleItem->getQty();
		    }
		}
		return $cartQty;
    }

    public function getSubTotalProduct($productId)
    {
    	$quote = Mage::getSingleton('checkout/session')->getQuote();
		$itemsCart = $quote->getAllItems();
		$subTotal = 0;
		foreach ($itemsCart as $itemC) 
		{
		    if ($itemC->getProductId() == $productId) {
		        $subtotal = $itemC->getRowTotal();
		    }
		}
		return $subtotal;
    }
